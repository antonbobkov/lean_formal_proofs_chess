...
-- BOARD BUILDER
-- ------------------------------------------------------------
-- A helper to construct a Board from a list of (sq4uare, piece) pairs.
-- Any sq4uare not in the list is empty (returns `none`).  The `turn`
-- parameter defaults to `.White` so the bulk of the existing tests
-- (which don't care about turn) keep working unchanged.
--
-- `List.find?` scans the list for a matching sq4uare.
...
def boardFrom (sq4uares : List (Pos 4 × Piece)) (turn : Color := .White) : Board 4 where
  pieces p := (sq4uares.find? fun (q, _) => q == p).map (·.2)
  turn := turn
...
#guard IsCheck (boardFrom [(sq4 0 0, WK), (sq4 0 3, BR)]) .White
...
#guard IsCheck (boardFrom [(sq4 0 0, WK), (sq4 3 0, BR)]) .White
...
#guard ¬ IsCheck (boardFrom [(sq4 0 0, WK), (sq4 0 1, WR), (sq4 0 3, BR)]) .White

-- Rook on a completely different row AND column — can't attack diagonally:
--   col  0  1  2  3
-- row 0 [WK  .  .   .]
-- row 1 [ .  .  .  BR]
#guard ¬ IsCheck (boardFrom [(sq4 0 0, WK), (sq4 1 3, BR)]) .White

-- Rook immediately adjacent (no sq4uares between them) → still CHECK.
-- `Between 1 2 x` is false for all x (no integer strictly between 1 and 2),
-- so nothing can block, and the attack succeeds.
#guard IsCheck (boardFrom [(sq4 1 1, WK), (sq4 1 2, BR)]) .White

-- A friendly (same-color) rook does NOT give check:
#guard ¬ IsCheck (boardFrom [(sq4 0 0, WK), (sq4 0 3, WR)]) .White
...
--   col  0  1
-- row 0 [WK BK]
#guard IsCheck (boardFrom [(sq4 0 0, WK), (sq4 0 1, BK)]) .White

-- Diagonal adjacency also counts:
--   col  0  1
-- row 0 [WK  .]
-- row 1 [ .  BK]
#guard IsCheck (boardFrom [(sq4 0 0, WK), (sq4 1 1, BK)]) .White

-- Kings two sq4uares apart — out of range:
#guard ¬ IsCheck (boardFrom [(sq4 0 0, WK), (sq4 0 2, BK)]) .White

-- Kings far apart:
#guard ¬ IsCheck (boardFrom [(sq4 0 0, WK), (sq4 2 3, BK)]) .White
...
#guard ¬ IsCheck (boardFrom [(sq4 0 3, BR)]) .White
...
#guard ¬ IsCheck (boardFrom [(sq4 3 3, BK), (sq4 0 3, BR)]) .White
...
#guard IsCheck (boardFrom [(sq4 0 0, BK), (sq4 0 3, WK), (sq4 3 0, WR)]) .Black

-- The same board is NOT check for White (white king is safe):
#guard ¬ IsCheck (boardFrom [(sq4 0 0, BK), (sq4 0 3, WK), (sq4 3 0, WR)]) .White
...
#guard IsLegalSetup (boardFrom [(sq4 0 0, WK), (sq4 3 3, BK)])

-- Kings adjacent horizontally → illegal (touching).
#guard ¬ IsLegalSetup (boardFrom [(sq4 0 0, WK), (sq4 0 1, BK)])

-- Kings adjacent diagonally → also illegal.
#guard ¬ IsLegalSetup (boardFrom [(sq4 0 0, WK), (sq4 1 1, BK)])

-- Missing black king → illegal.
#guard ¬ IsLegalSetup (boardFrom [(sq4 0 0, WK)])

-- Missing white king → illegal.
#guard ¬ IsLegalSetup (boardFrom [(sq4 3 3, BK)])

-- Two white kings → illegal.
#guard ¬ IsLegalSetup (boardFrom [(sq4 0 0, WK), (sq4 2 0, WK), (sq4 3 3, BK)])
...
#guard IsLegalSetup
  (boardFrom [(sq4 0 0, BK), (sq4 3 0, WR), (sq4 3 3, WK)] .Black)
...
#guard ¬ IsLegalSetup
  (boardFrom [(sq4 0 0, BK), (sq4 3 0, WR), (sq4 3 3, WK)] .White)
...
-- checks along row 0.  BK has three candidate sq4uares:
...
-- row 2 [WR  .  .   .]
-- row 3 [ .  .  WK  .]
#guard IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 0 3, WR), (sq4 1 0, WR), (sq4 2 0, WR), (sq4 3 2, WK)]) .Black

-- White is not in checkmate on the same board (white king is never in check):
#guard ¬ IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 0 3, WR), (sq4 1 0, WR), (sq4 2 0, WR), (sq4 3 2, WK)]) .White

-- In check but can escape: BK moves to (1,0) which nothing attacks.
#guard ¬ IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 0 3, WR), (sq4 3 3, WK)]) .Black

-- Not in check → cannot be checkmate.
#guard ¬ IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 3 3, WK)]) .Black

-- In check but BK can capture the attacker: after BK takes WR at (1,0),
-- the white king at (3,3) is too far to threaten (1,0).
#guard ¬ IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 1 0, WR), (sq4 3 3, WK)]) .Black
...
#guard ¬ IsCheckmate (boardFrom [(sq4 0 0, BK), (sq4 0 3, WR), (sq4 1 3, WR), (sq4 3 0, WK), (sq4 2 1, BR)]) .Black
